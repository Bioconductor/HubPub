...
is `mypackage` then you would use the following call:

```
storage_multiupload(container, src = "/home/user/mypackage/data/*", dest = "mypackage", recursive=TRUE)
```
Please make sure the `dest` value is the name of your package you will be
submitting. Subdirectories, if utilized, should upload automatically by using `*`
in combination with `recursive=TRUE`. 

For data that is currently being stored remotely, use `copy_url_to_storage` or `multicopy_url_to_storage`.
...
```
azcopy --recursive MyPackageData <sas url>
```
...
Once Installed, open the storage explorer and follow the following steps.
     ...
     Container`. If the windows does not automatically appear, see
     Troubleshooting GUI at the bottom of this section for instructions on how
     to make this window appear, how to navigate if already logged in with a
     valid, non-expired sas token, and what to do if your sas token has expired.

     2. In the `Select Connection Method` window, select `Shared access
     signature URL (SAS)` and click Next in the bottom right corner.
